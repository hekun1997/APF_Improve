...
def fit_fun(start, end, obs_XY, enemy_XY,X):  # 适应函数
    return de_apf(start, end, obs_XY, enemy_XY, X[0], X[1], X[2], X[3])


class Unit:
    # 初始化
    def __init__(self,start, end, obs_XY, enemy_XY, x_min, x_max, dim):
        self.__pos = np.array([x_min[i] + random.random()*(x_max[i] - x_min[i]) for i in range(dim)])   # 在这里可以写一些约束条件
        self.__mutation = np.array([0.0 for i in range(dim)])  # 个体突变后的向量
        self.__crossover = np.array([0.0 for i in range(dim)])  # 个体交叉后的向量
        self.__fitnessValue = fit_fun(start, end, obs_XY, enemy_XY, self.__pos)  # 个体适应度
...
class DE:
    def __init__(self, start, end, obs_XY, enemy_XY, dim, size, iter_num, x_min, x_max, best_fitness_value=float('Inf'), F=0.5, CR=0.8):
        self.F = F
        self.CR = CR
        self.dim = dim  # 维度
        self.size = size  # 总群个数
        ...
        # 对种群进行初始化
        self.unit_list = [Unit(start, end, obs_XY, enemy_XY, self.x_min, self.x_max, self.dim) for i in range(self.size)]
    ...
    def mutation_fun(self):
        for i in range(self.size):
            ...
            for j in range(self.dim):
                ...
                if self.x_min[j] <= mutation[j] <= self.x_max[j]:
                    self.get_kth_unit(i).set_mutation(j, mutation[j])
                else:
                    rand_value = self.x_min[j] + random.random()*(self.x_max[j] - self.x_min[j])
    ...
    # 选择
    def selection(self,start, end, obs_XY, enemy_XY):
        for unit in self.unit_list:
            new_fitness_value = fit_fun(start, end, obs_XY, enemy_XY, unit.get_crossover())
    ...
    def update(self,start, end, obs_XY, enemy_XY):
        for i in range(self.iter_num):
            self.mutation_fun()
            self.crossover()
            self.selection(start, end, obs_XY, enemy_XY)
